...
const menuright = [
  {route:"/cart", icon:<ShoppingCart size={28}/>},
      ...
      <nav className='w-full'>
        <div className="px-2 flex  justify-between items-center">
          ...
          <div className='flex items-center gap-5'>
            {menuright.map((m)=>(
               <Link to="/cart" onClick={()=>setActive(m.route)} className={`${active==m.route?"text-white bg-amber-500 shadow-md":"text-amber-500"} rounded-full   p-2 cursor-pointer hover:bg-amber-500 hover:text-white  transition-all duration-300`}>
                {m.icon}
               </Link>
            ))}
